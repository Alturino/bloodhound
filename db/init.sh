...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<EOSQL
  ...
  CREATE USER ${POSTGRES_BLOODHOUND_PDF_EXTRACTOR_USER} WITH PASSWORD '${POSTGRES_BLOODHOUND_PDF_EXTRACTOR_PASSWORD}';
  ...
  CREATE USER ${POSTGRES_BLOODHOUND_ANALYZER_USER} WITH PASSWORD '${POSTGRES_BLOODHOUND_ANALYZER_PASSWORD}';
